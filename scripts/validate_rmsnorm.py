#!/usr/bin/env python3
"""Cross-implementation RMSNorm validation: Odyssey (PyTorch) vs Phalanx (Rust).
...
Procedure
1. Generate identical random ``x`` and ``γ`` (float32, fixed seed).
2. Apply Odyssey ``OdysseyRMSNorm``.
3. Serialize inputs + manifest for the Phalanx ``validate_rmsnorm`` binary.
4. Run Phalanx RMSNorm on the same tensors.
5. Compare max / mean absolute error against a tolerance (default 1e-6).

Example
-------
    python scripts/validate_rmsnorm.py
    python scripts/validate_rmsnorm.py --tolerance 1e-6 --seed 0
"""
...
DEFAULT_REPORT = REPO_ROOT / "experiments" / "ODY-0005" / "rmsnorm_validation.json"
...
def run_phalanx(
    ...
    *,
    ...
    cmd = ["cargo", "run", "--quiet", "--bin", "validate_rmsnorm"]
    if release:
        cmd.append("--release")
...
def main() -> int:
    ...
    gamma = torch.randn(args.hidden_size, dtype=torch.float32) * 0.1 + 1.0
    ...
    y_ody = run_odyssey(x, gamma, config)

    work = (
        Path(tempfile.mkdtemp(prefix="rmsnorm_val_"))
    ...
    _write_f32(work / "gamma.bin", gamma.numpy())
    ...
    report = {
        ...
        "message": (
            ...
            if passed
            else "RMSNorm outputs diverge beyond tolerance."
        ),
    }
